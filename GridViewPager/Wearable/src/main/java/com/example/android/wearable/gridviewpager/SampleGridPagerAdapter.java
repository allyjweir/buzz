...
import com.example.android.wearable.gridviewpager.fragments.BuzzFragment;
import com.example.android.wearable.gridviewpager.fragments.CallFragment;
import com.example.android.wearable.gridviewpager.fragments.DistanceFragment;
import com.example.android.wearable.gridviewpager.fragments.StatusFragment;
import com.example.android.wearable.gridviewpager.fragments.TextFragment;
...
//    };
    ...
    private class Row {
        ...
        public Row(int contact){
            Bundle bundle = new Bundle();
            bundle.putInt("contact_number", contact);

            DistanceFragment distance = new DistanceFragment();
            distance.setArguments(bundle);
            add(distance);

            BuzzFragment buzz = new BuzzFragment();
            buzz.setArguments(bundle);
            add(buzz);
...
//    }
    ...
    private void addContactRows(){
        int contactsSize = UserSettings.getInstance(mContext).getContacList().size();
        for (int index = 0; index < contactsSize; index++){
            mRows.add(new Row(index));
        }
    }
}
